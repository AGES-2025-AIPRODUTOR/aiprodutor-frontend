...
export default function Relatorios() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const culturaNorm: TotalCultura[] = porCulturaRes.map(
          (c: { culture: any; value: any }) => ({
            cultura: c.culture,
            kg: Number(c.value ?? 0),
          })
        );
  ...
  return (
    <>
      <PageTitle title="Relatórios" href="/" variant="center" />
      <div className="p-4 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 opacity-100">
          <CartaoKPI
            ...
            valor={kpi?.areaHa ?? (loading ? '...' : '--')}
            unidade="ha"
          />
          <CartaoKPI
            titulo="Culturas Ativas"
            valor={kpi?.culturasAtivas ?? (loading ? '...' : '--')}
          />
          <CartaoKPI
          ...
          <CartaoKPI
            ...
            valor={kpi?.eficienciaKgHa ?? (loading ? '...' : '--')}
            unidade="kg/ha"
          />
        </div>
